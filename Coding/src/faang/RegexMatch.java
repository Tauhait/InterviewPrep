...
//Implementing a regex match
/*
 * This is an easy string DP problem that is basically derived from classics such as Edit Distance, Longest Common SubString, etc. 
 */
public class RegexMatch {	
	public boolean isMatchRecursive(String text, String pattern) {
        ...
        //Time & Space complexity - O((m + n)* 2 ^ (m + n))
    }
	
	/* You don't need to do the DP solution. Do the recursive solution and then memoize it.
		I think that is exactly how to implement top-down dp :). 
	*/
	/* Top down DP Soln */
	...
	if (map.containsKey(key)) return map.get(key);
	    ...
	    if (pattern.length() >= 2 && pattern.charAt(1) == '*'){
	        boolean val = (isMatch(text, pattern.substring(2)) ||
	                (first_match && isMatch(text.substring(1), pattern)));
	        map.put(key, val);
	        return val;
	    } else {
	        boolean val = first_match && isMatch(text.substring(1), pattern.substring(1));
	        map.put(key, val);
	        return val;
	    }
	}
